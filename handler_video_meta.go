...
func (cfg *apiConfig) handlerVideoGet(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respondWithError(w, http.StatusInternalServerError, "Error when fetching pre signed url for video", err)
		return
	}
...
func (cfg *apiConfig) handlerVideosRetrieve(w http.ResponseWriter, r *http.Request) {
	...
	for _, v := range videos {
		presignedVideo, err := cfg.dbVideoToSignedVideo(v)
		if err != nil {
			respondWithError(w, http.StatusInternalServerError, "Error when fetching presigned video", err)
			return
		}
